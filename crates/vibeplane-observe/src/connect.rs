...
//!
//! This is the only part of Vibeplane that writes to a file the user owns, so
//! it is the part that has to be most careful. Four rules:
//!
...
///
/// `PermissionRequest` is synchronous because it is the instant signal that a
/// session is blocked — the `permission_prompt` notification waits six seconds
/// and, in a terminal, defers on every keystroke. Everything else is `async` so
/// that no hook can ever make Claude feel slower.
///
...
pub fn connect(
    ...
    for (event, matcher, is_async) in HOOKS {
        let path = if *event == "PermissionRequest" {
            "policy"
        } else {
            "hook"
        };
...
mod tests {
    ...
    #[test]
    fn only_the_policy_hook_is_synchronous() {
        let s = connected();
        for (event, entries) in s["hooks"].as_object().unwrap() {
            let hook = &entries[0]["hooks"][0];
            if event == "PermissionRequest" {
                assert!(hook.get("async").is_none(), "the policy hook must block");
                assert!(hook.get("timeout").is_some(), "and must have a deadline");
            } else {
