...
use vibeplane_domain::attention::{AttentionItem, Level};
...
mod tests {
    use super::*;
    use vibeplane_domain::attention::{Action, AttentionKind};
    use vibeplane_domain::ids::{AttentionId, RunId};
    ...
    fn quiet_items_never_interrupt() {
        ...
        n.sync(&[item("a", Level::Normal), item("b", Level::Low)]);
        assert!(
