//! Declared chains of agent runs (D34).
//!
...
use std::path::Path;
use vibeplane_core::config::{Expect, ProjectConfig, RoleStep, Step};
use vibeplane_domain::ids::WorkId;
use vibeplane_domain::work::{Phase, Pipeline};

/// The roles of a declared pipeline, in order, for copying onto a Work.
pub fn roles(p: &vibeplane_core::config::Pipeline) -> Vec<String> {
    p.steps
        .iter()
        .map(|s| match s {
            Step::Human(h) => h.human.clone(),
            Step::Role(r) => r.role.clone(),
        })
        .collect()
}
...
fn step_at<'a>(config: &'a vibeplane_core::config::Pipeline, role: &str) -> Option<&'a Step> {
    config.steps.iter().find(|s| match s {
        Step::Human(h) => h.human == role,
        Step::Role(r) => r.role == role,
    })
}

/// Builds the text a step's agent is given.
///
/// `prompt` names a template in `.vibeplane/prompts/<name>.md` — committed on
/// the branch, like the gates — and is used literally when no such file exists.
/// That fallback is deliberate: a one-line pipeline should not require creating
/// a directory of files first.
fn compose(dir: &Path, step: &RoleStep, title: &str, task: &str, findings: Option<&str>) -> String {
    let path = dir
        .join(".vibeplane/prompts")
        .join(format!("{}.md", step.prompt));
    let base = std::fs::read_to_string(&path).unwrap_or_else(|_| step.prompt.clone());
...
fn start_step<'a>(
    ...
    declared: &'a vibeplane_core::config::Pipeline,
...
async fn start_step_inner(
    ...
    declared: &vibeplane_core::config::Pipeline,
) -> Result<()> {
    ...
    let run_id = crate::driven::dispatch(state, &spec, dir, Some(text)).await?;
    ...
    state.store.save_work(&saved).await.ok();
    state.work_of_run.lock().await.insert(run_id, id.clone());
...
pub async fn on_turn_ended(state: &Shared, id: &WorkId) {
    if let Err(e) = advance(state, id).await {
        ...
        crate::work::set_phase(state, id, Phase::Failed).await;
    }
}
...
async fn advance(state: &Shared, id: &WorkId) -> Result<()> {
    ...
    if let Some(gate) = &step.gate {
        ...
        let report = vibeplane_gates::run_expecting(
            gate,
            &commands,
            &dir,
            timeout,
...
pub async fn begin(state: &Shared, id: &WorkId, config: &ProjectConfig, name: &str) -> Result<()> {
    ...
    let first = role_list.first().cloned().context("`{name}` is empty")?;
    {
        let mut works = state.works.lock().await;
        let w = works.get_mut(id).context("no such work")?;
...
mod tests {
    use super::*;
    use vibeplane_core::config::Findings;
