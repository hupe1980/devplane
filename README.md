...
has editor tabs whose processes are still alive, and on the machine this was built against 23
sessions were listed and five were in play. A dormant session that starts asking for something
...
vibeplane ls --project mako  # one project; matches on any part of the name
...
| `crates/vibeplane-store` | SQLite: events, runs, work, full-text search |
...
| **M3 Pipelines** | implement → review → verify, with agents checking agents | ⏳ chains, findings loops and human steps run; the durable runtime beneath them is designed |

## Development

```sh
cargo build --examples       # builds the fixture agent the ACP suite drives
cargo test                   # 201 tests, no network, no provider, no bill
