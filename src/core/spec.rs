...
impl Counts {
    /// *11 ticked · 9 verified*, *… · no gates declared*, or *… · stale* when
    /// the verifying gate ran against a tree that has since moved.
    pub fn says(&self, stale: bool) -> String {
        match self.verified_tasks {
            None => format!("{} ticked · no gates declared", self.ticked),
            Some(v) if stale => format!("{} ticked · {v} verified · stale", self.ticked),
            Some(v) => format!("{} ticked · {v} verified", self.ticked),
        }
    }
}
...
pub fn counts(
    ...
    let verified = ticked
        .iter()
        .filter(|t| is_verified(runs, &t.key(), last_pass))
        .count();
    ...
    Counts {
        ...
        verified_tasks: gates_declared.then_some(verified as u32),
        ticked_unsent,
        sent_unticked,
    }
}
...
pub fn token_rows(
    ...
    trace
        ...
        .map(|(token, tasks)| {
            ...
            let verified = keys
                .iter()
                .filter(|(k, done)| *done && is_verified(runs, k, last_pass))
                .count();
            TokenRow {
                token: token.clone(),
...
mod edge_counts {
    ...
    #[test]
    fn eleven_ticked_nine_verified_over_two_runs() {
        let t = fixture();
        ...
        assert_eq!(c.says(true), "11 ticked · 9 verified · stale");

        // The gate passed before the second run closed: its four are not verified.
        let early = Some(at("2026-09-25T10:02:00Z"));
        let c = counts(&t, &[&a, &b], true, early);
        assert_eq!((c.ticked, c.verified_tasks), (11, Some(6)));
        ...
        let flat: Vec<(String, u32, u32, Option<u32>)> = rows
            .iter()
            .map(|r| (r.token.clone(), r.tasks, r.ticked, r.verified_tasks))
            .collect();
        assert_eq!(
            flat,
            [
        ...
        assert_eq!(rows[3].says(), "4 tasks · 2 ticked · 0 verified");
        assert_eq!(
