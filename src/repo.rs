//! Where a directory belongs: the repository, worktree owner and governing
//! root of a path, read off the disk. Outside the pure half because every
//! answer here is a filesystem read; `src/core/` takes the answer as a value.

use crate::core::world::{Roots, World};
use std::path::{Path, PathBuf};

impl World {
    /// A world that resolves projects on disk: a directory as the
    /// filesystem names it, governed by [`governing_root`].
    pub fn new() -> Self {
        Self::with_roots(Roots(project_root))
    }
}

/// The root whose project a directory belongs to, as the filesystem names
/// it. `None` only for a path that names nothing on disk at all.
pub fn project_root(path: &Path) -> Option<PathBuf> {
    let named = named(path);
    Some(governing_root(&named).unwrap_or(named))
}
...
/// * A repository nested in another (a vendored clone, or a `.git` an agent
///   made with `mkdir`) does not escape it: the outermost repository on the
///   way up that carries a `devplane.toml` governs. Without one, the nearest
///   repository does, so a dotfiles repository at `~` claims nothing.
pub fn governing_root(path: &Path) -> Option<PathBuf> {
    ...
    let config = |d: &Path| d.join(crate::core::config::CONFIG_FILE).is_file();
    let mut config_root = None;
    let mut nearest_repo: Option<PathBuf> = None;
    let mut configured_repo: Option<PathBuf> = None;
    let mut cur = Some(path);
    while let Some(d) = cur {
        let dot_git = d.join(".git");
        let repo = if dot_git.is_dir() {
            Some(d.to_path_buf())
        ...
        match repo {
            Some(r) => {
                if config(&r) {
                    configured_repo = Some(r.clone());
                }
                nearest_repo.get_or_insert(r);
            }
            None if config_root.is_none() && nearest_repo.is_none() && config(d) => {
    ...
    configured_repo.or(nearest_repo).or(config_root)
}
